...
class DynamicSolver(fe.Solver):
    def __init__(self, model, **kwargs):
        ...
        self._alpha = opt.pop("alpha", 1.0 / 2.0)
        self._beta = opt.pop("beta", 1.0 / 2.0)
        ...
        self._coeff = {
            "disp": {
                "disp": 1.0,
                "velo": 1.0 / (self._alpha * self._delta_t),
                "acce": 1.0 / (self._alpha * self._beta * self._delta_t**2),
            },  # NOQA: E501
            "velo": {
                "disp": self._alpha * self._delta_t,
                "velo": 1.0,
                "acce": 1.0 / (self._beta * self._delta_t),
            },
            "acce": {
                "disp": self._alpha * self._beta * self._delta_t**2,  # NOQA: E501
    ...
    def _predictor(self):
        self._model.u += (
            self._delta_t * self._model.v
            + self._delta_t**2 / 2.0 * self._model.a
        )
    ...
    def solveStep(self):
        ...
        while not converged:
            ...
            converged = (
                error < self._tolerance or self._nloops > self._max_nloops
            )
